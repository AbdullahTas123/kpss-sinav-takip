...
function QuestionsPage() {
  ...
  const [chartRange, setChartRange] = usePersistentState("questions.chartRange", "14g");
  ...
  const confirmDelete = () => {
    if (pendingDeleteId) kpssStore.deleteQuestion(pendingDeleteId);
...
function binAxisLabel(b, mode) {
  if (mode === "day") {
    return b.date.toLocaleDateString("tr-TR", { day: "2-digit", month: "short", timeZone: TR_TZ });
  }
  if (mode === "week") {
    const sd = new Date(b.startKey + "T12:00:00+03:00");
    return sd.toLocaleDateString("tr-TR", { day: "2-digit", month: "short", timeZone: TR_TZ });
  }
  return b.date.toLocaleDateString("tr-TR", { month: "short", timeZone: TR_TZ });
}

function binTooltipLabel(b, mode) {
  if (mode === "day") {
    return b.date.toLocaleDateString("tr-TR", { weekday: "short", day: "2-digit", month: "long", timeZone: TR_TZ });
  }
  if (mode === "week") {
    const sd = new Date(b.startKey + "T12:00:00+03:00");
    const ed = new Date(b.endKey + "T12:00:00+03:00");
    const sStr = sd.toLocaleDateString("tr-TR", { day: "2-digit", month: "short", timeZone: TR_TZ });
    const eStr = ed.toLocaleDateString("tr-TR", { day: "2-digit", month: "short", timeZone: TR_TZ });
    return `${sStr} – ${eStr}`;
  }
  return b.date.toLocaleDateString("tr-TR", { month: "long", year: "numeric", timeZone: TR_TZ });
}
...
function QuestionChart({ questions, isMobile, range }) {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      // Container'ın gerçek genişliğini ölç — sabit minimum yok ki dar mobilde
      // SVG container'dan taşmasın (eskiden 320 min yatay scroll'a sebep oluyordu).
      for (const e of entries) setWidth(Math.max(1, e.contentRect.width));
    });
    ro.observe(wrapRef.current);
    return () => ro.disconnect();
  }, []);

  const { bins, mode } = useMemo(() => buildBins(questions, range), [questions, range]);
...
function QuestionModal({ existing, onClose }) {
  ...
  const [wrong, setWrong] = useState(existing ? String(existing.wrong) : "");
  ...
  useEffect(() => { setError(""); }, [total, correct, wrong, subject, topic]);
  ...
  const onSave = () => {
    ...
    if (existing) kpssStore.updateQuestion(existing.id, payload);
    else kpssStore.addQuestion(payload);
    onClose();
  };
