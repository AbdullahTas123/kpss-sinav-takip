    ...
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block" }}>
      ...
      {points.map((p, i) => (
        <g key={i}>
          ...
          <title>{`${fmtDate(p.date)}: ${round2(p.net)} net`}</title>
        </g>
      ))}
...
function Dashboard() {
  ...
  const [weakFilter, setWeakFilter] = usePersistentState("dashboard.weakFilter", "all");
  const [weakSource, setWeakSource] = usePersistentState("dashboard.weakSource", "tum");
  ...
  const dersLbl = weakFilter === "all" ? "" : ` ${KPSS_SUBJECTS[weakFilter].name} için`;
      ...
      <Card style={{ padding: isMobile ? 14 : 16, marginBottom: isMobile ? 12 : 14 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
            ...
            {Object.keys(KPSS_SUBJECTS).map((sk) => {
              const s = KPSS_SUBJECTS[sk];
              return (
                <Chip
                  key={sk}
