...
function TopicsPage() {
  const { Card, Button, Icon, ConfirmModal } = window.KPSS_UI;
  ...
  const [activeSubj, setActiveSubjRaw] = window.usePersistentState("topics.activeSubj", subjectKeys[0]);
  const setActiveSubj = (sk) => {
    if (subjectKeys.includes(sk)) setActiveSubjRaw(sk);
    else setActiveSubjRaw(subjectKeys[0]);
  };
  const [csvOpen, setCsvOpen] = React.useState(false);
  const [pendingPlDelete, setPendingPlDelete] = React.useState(null);
  ...
  const cycleStatus = (topic) => {
    ...
    window.kpssStore.setTopicStatus(activeSubj, topic, next);
  };

  const confirmPlDelete = () => {
    if (pendingPlDelete) window.kpssStore.deletePlaylist(pendingPlDelete);
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {subjPlaylists.map((pl) => (
                <PlaylistRow
                  ...
                  onToggleVideo={(idx) => window.kpssStore.togglePlaylistVideo(pl.id, idx)}
                  onMarkAll={(watched) => window.kpssStore.setPlaylistAllWatched(pl.id, watched)}
                  onRename={(newName) => window.kpssStore.renamePlaylist(pl.id, newName)}
                  onToggleSelected={() => window.kpssStore.setPlaylistSelected(pl.id, !pl.selected)}
                />
              ))}
            </div>
          </>
...
function PlaylistRow({ playlist, color, expanded, onToggle, onDelete, onToggleVideo, onMarkAll, onRename, onToggleSelected }) {
  ...
  React.useEffect(() => { setName(playlist.name); }, [playlist.name]);
...
function CsvUploadModal({ defaultSubject, onClose }) {
  const { Button, Input, Select, Label, Icon } = window.KPSS_UI;
  const SUBJ = window.KPSS_SUBJECTS;
  const subjectKeys = Object.keys(SUBJ);
  const { isMobile } = window.useViewport();

  const [subject, setSubject] = React.useState(defaultSubject || subjectKeys[0]);
  const [name, setName] = React.useState("");
  const [parsed, setParsed] = React.useState(null); // { videos, fileName }
