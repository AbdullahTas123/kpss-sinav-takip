...
function ExamChart({ exams, isMobile, kind = "genel", goals }) {
  const { Icon, calcNet, round2, fmtDate, fmtDateShort } = window.KPSS_UI;
  const SUBJ = window.KPSS_SUBJECTS;
  const subjectKeys = Object.keys(SUBJ);
  const isBrans = kind === "brans";

  const [range, setRange] = window.usePersistentState("examchart.range", "last10");
  const [mode, setMode] = window.usePersistentState("examchart.mode", kind === "brans" ? "subjects" : "total"); // 'total' | 'subjects'
  const [metric, setMetric] = window.usePersistentState("examchart.metric", "net"); // 'net' | 'duration'
  ...
  const [wrapWidth, setWrapWidth] = React.useState(800);
  ...
  const sourceExams = React.useMemo(
...
window.KPSS_EXAM_CHART = ExamChart;
