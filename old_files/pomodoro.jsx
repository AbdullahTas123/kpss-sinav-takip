...
function PomodoroPage() {
  const { Card, Button, Icon } = window.KPSS_UI;
  ...
  const [mode, setMode] = window.usePersistentState("pomodoro.mode", "pomodoro");
  const [timeLeft, setTimeLeft] = React.useState(() => (MODES[mode] || MODES.pomodoro).duration);
  const [isRunning, setIsRunning] = React.useState(false);
  const [completed, setCompleted] = React.useState(0);

  // Mola sonrası dönülecek son çalışma modu — pomodoro mu uzun mu seçtiyse onu hatırla.
  const lastWorkRef = React.useRef(MODES[mode]?.kind === "work" ? mode : "pomodoro");
  React.useEffect(() => {
      ...
      setTimeLeft((t) => {
        if (t <= 1) {
          ...
          if (cur && cur.kind === "work") {
            setCompleted((c) => {
              const nextCount = c + 1;
              // const next = nextCount % 4 === 0 ? "long" : "short";
...
window.KPSS_POMODORO = PomodoroPage;
