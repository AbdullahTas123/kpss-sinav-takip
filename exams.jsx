...
function ExamsPage() {
  const { Card, Button, Input, Select, Label, Chip, Tabs, Icon, ConfirmModal, calcNet, round2, fmtDate } = window.KPSS_UI;
  ...
  const [filterType, setFilterType] = window.usePersistentState("exams.filterType", "all"); // all | genel | brans
  const [chartKind, setChartKind] = window.usePersistentState("exams.chartKind", "genel"); // genel | brans
  ...
  const confirmDelete = () => {
    if (pendingDeleteId) window.kpssStore.deleteExam(pendingDeleteId);
  ...
  const avgGeneralNet = React.useMemo(() => {
  ...
  return (
    <div style={{ padding: isMobile ? "20px 16px 60px 16px" : "28px 32px 64px 32px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <Card style={{ padding: isMobile ? 16 : 24, marginBottom: 16 }}>
        ...
        <window.KPSS_EXAM_CHART exams={state.exams} isMobile={isMobile} kind={chartKind} goals={state.goals} />
      </Card>
...
function ExamModal({ existing, onClose }) {
  const { Card, Button, Input, Select, Label, Chip, Tabs, Icon, calcNet, round2 } = window.KPSS_UI;
  const SUBJ = window.KPSS_SUBJECTS;
  const subjectKeys = Object.keys(SUBJ);
  const { isMobile } = window.useViewport();

  const [tab, setTab] = React.useState(existing?.type || "genel");
  const [name, setName] = React.useState(existing?.name || "");
  const [date, setDate] = React.useState(existing ? window.KPSS_UI.trDateKey(existing.date) : window.KPSS_UI.trToday());
  const [duration, setDuration] = React.useState(existing?.durationMin ? String(existing.durationMin) : "");
  ...
  const [genel, setGenel] = React.useState(initialGenel);

  // Branş — toplam soru ders sabiti (fixedCount), kullanıcı girmez
  const [bSubject, setBSubject] = React.useState(existing?.subject || "");
  const [bCorrect, setBCorrect] = React.useState(existing?.correct ? String(existing.correct) : "");
  const [bWrong, setBWrong] = React.useState(existing?.wrong ? String(existing.wrong) : "");
  const [bWeak, setBWeak] = React.useState(existing?.weakTopics || []);
  ...
  React.useEffect(() => { setBWeak(existing && existing.subject === bSubject ? existing.weakTopics || [] : []); }, [bSubject]);
  ...
  const onSave = () => {
    ...
    const baseDate = existing && window.KPSS_UI.trDateKey(existing.date) === date ? existing.date : newIso;
    ...
    if (tab === "genel") {
      ...
      if (existing) window.kpssStore.updateExam(existing.id, payload);
      else window.kpssStore.addExam(payload);
    } else {
