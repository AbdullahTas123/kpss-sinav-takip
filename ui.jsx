...
const calcNet = (correct, wrong) => Math.max(0, correct - wrong / 4);
...
// Button
function Button({ children, variant = "primary", size = "md", onClick, disabled, type = "button", style = {}, className = "" }) {
  const base = {
...
// Input
function Input({ value, onChange, placeholder, type = "text", min, max, error, style = {}, ...props }) {
  return (
    <input
      type={type}
...
// Badge / topic chip
function Chip({ children, active, onClick, color = "#10b981", style = {} }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function ConfirmModal({ title, message, confirmLabel = "Onayla", cancelLabel = "Vazgeç", onConfirm, onCancel, danger = false }) {
  React.useEffect(() => {
...
window.KPSS_UI = { Card, Button, Input, Select, Label, Chip, Tabs, Icon, ConfirmModal, formatRange, fmtDate, fmtDateShort, calcNet, round2, TR_TZ, trDateKey, trToday, trAddDays, trMakeIso };
