...
function useCountdown(targetIso) {
  const [now, setNow] = useStateShell(Date.now());
  useEffectShell(() => {
...
function useViewport() {
  const [w, setW] = useStateShell(typeof window !== "undefined" ? window.innerWidth : 1280);
  useEffectShell(() => {
...
function DataActions() {
  const { Icon } = window.KPSS_UI;
  const fileRef = useRefShell(null);
  ...
  const onImport = async (e) => {
    ...
    try {
      const r = await window.kpssStore.importJSON(f);
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        <button style={btn} onClick={() => window.kpssStore.exportJSON()} title="JSON olarak indir">
...
function Topbar({ examDate, applyDateStart, applyDateEnd, onMenu }) {
  const { Icon } = window.KPSS_UI;
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              ...
              <span style={{ fontSize: 13, color: "#d4d4d8", fontWeight: 500 }}>
                {window.KPSS_UI.formatRange(applyDateStart, applyDateEnd)}
...
window.KPSS_SHELL = { Sidebar, Topbar };
